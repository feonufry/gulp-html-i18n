(function() {
  ...
  getProperty = function(file, propName, properties, opt, fallbackTried) {
    var res, tmp;
    tmp = propName.split('.');
    res = properties;
  ...
  handleUndefined = function(file, propName, properties, opt, fallbackTried) {
    var lang;
    lang = fallbackTried ? opt.fallback : properties._lang_;
    if (opt.failOnMissing && (!opt.fallback || fallbackTried)) {
      console.error(gutil.colors.red(("Error at " + (gutil.colors.white(file.path)) + ": `" + (gutil.colors.white(propName)) + "` not found in definition file for ") + (opt.fallback && properties._lang_ !== opt.fallback ? "`" + (gutil.colors.white(properties._lang_)) + "/" + (gutil.colors.white(opt.fallback)) + "` locales." : "`" + (gutil.colors.white(lang)) + "` locale.")));
      throw gutil.colors.red("Localization was terminated: Undefined key was found, see above.");
    } else {
      if (!opt.fallback || fallbackTried) {
        return console.warn(gutil.colors.yellow(("Warning at " + (gutil.colors.white(file.path)) + ": : `" + (gutil.colors.white(propName)) + "` not found in definition file for ") + (opt.fallback && properties._lang_ !== opt.fallback ? "`" + (gutil.colors.white(properties._lang_)) + "/" + (gutil.colors.white(opt.fallback)) + "` locales." : "`" + (gutil.colors.white(lang)) + "` locale.")));
      }
    }
  };

  replaceProperties = function(file, content, properties, opt, lv, fallbackTried) {
    var langRegExp;
    lv = lv || 1;
    ...
    return through.obj(function(file, enc, next) {
      ...
      return getLangResource(langDir, opt).then((function(_this) {
        return function(langResource) {
          var content;
          if (file._lang_) {
            content = replaceProperties(file, file.contents.toString(), extend({}, langResource[file._lang_], {
              _lang_: file._lang_,
            ...
            langResource.LANG_LIST.forEach(function(lang) {
              ...
              if (opt.createLangDirs) {
                ...
                if (opt.filenameI18n) {
                  newFilePath = replaceProperties(file, newFilePath, extend({}, langResource[lang], {
                    _lang_: lang,
                    _default_lang_: opt.defaultLang || ''
                  }), opt);
                }
              ...
              content = replaceProperties(file, file.contents.toString(), extend({}, langResource[lang], {
                _lang_: lang,
                _default_lang_: opt.defaultLang || ''
              }), opt);
              if (opt.fallback) {
                content = replaceProperties(file, content, extend({}, langResource[opt.fallback], {
                  _lang_: lang,
                  _default_lang_: opt.defaultLang || ''
                }), opt, void 0, true);
              }
